/* eslint-disable */
/**
 * Site interactions — ported from the static build's assets/js/main.js.
 *
 * Call initInteractions() from a client component; it returns a disposer.
 * Everything registered during init (DOM listeners and IntersectionObservers)
 * is tracked, so a client-side navigation can tear the previous page's
 * behaviour down and wire the new one up cleanly.
 *
 * The mobile drawer is NOT here — components/Drawer.tsx owns that state.
 */
export function initInteractions() {
  var ac = new AbortController();
  var observers = [];

  // Briefly wrap addEventListener / IntersectionObserver so that everything the
  // (synchronous) body below registers is attached to this run's abort signal.
  var nativeAdd = EventTarget.prototype.addEventListener;
  var NativeIO = window.IntersectionObserver;

  EventTarget.prototype.addEventListener = function (type, handler, options) {
    var opts;
    if (options === true || options === false) opts = { capture: options };
    else if (options && typeof options === "object") opts = Object.assign({}, options);
    else opts = {};
    opts.signal = ac.signal;
    return nativeAdd.call(this, type, handler, opts);
  };

  function TrackedIO(cb, opts) {
    var io = new NativeIO(cb, opts);
    observers.push(io);
    return io;
  }
  TrackedIO.prototype = NativeIO.prototype;
  ...
  /* ==========================================================================
    ...
    function onScroll() {
      ...
      if (header) {
        ...
        header.classList.toggle("is-hidden", goingDown && !document.documentElement.classList.contains("nav-open"));
      }
  ...
  /* ==========================================================================
    ...
    (function lightbox() {
      ...
      function openAt(i) {
        ...
        show(i);
        root2.classList.add("lightbox-open");
      ...
      function close() {
        ...
        box.setAttribute("aria-hidden", "true");
        root2.classList.remove("lightbox-open");
